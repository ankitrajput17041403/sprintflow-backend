...
public class ActivityServiceImpl implements ActivityService {
    ...
    @Override
    public void logActivity(Issue issue, User performedBy, ActivityAction action, String description) {
    ...
    public List<ActivityResponse> getActivitiesByIssue(Long issueId) {
        ...
        return activityRepository.findByIssueIdOrderByCreatedAtDesc(issueId).stream().
                map(activity -> ActivityResponse.builder().id(activity.getId()).
                        action(activity.getAction()).
                        description(activity.getDescription()).
                        performedBy(activity.getPerformedBy() !=null
                                ?activity.getPerformedBy().getName()
                                :null).
                        performedByEmail(activity.getPerformedBy().getEmail()).
